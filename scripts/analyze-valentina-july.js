...
async function main() {
  ...
  try {
    ...
    const cots = await conn.query(`
      SELECT cot.id, cot.codigo, cot.monto, cot.fecha, cot.estado, c.id as clienteId, c.empresa, c.asignadoA
      FROM cotizacion cot
    ...
    cots.forEach(c => {
      console.log(`   - [${c.codigo}] ${c.empresa} | Monto: S/ ${c.monto} | Fecha: ${c.fecha} | Estado: ${c.estado}`);
    });
    ...
    console.log(`\n3. Clientes únicos atendidos en el período: ${clienteIds.size}`);

    console.log('\n--- MUESTRA DE LÍNEA DE TIEMPO COMPLETA DE CLIENTES (DESDE SU PROSPECCIÓN) ---');
    for (const cId of Array.from(clienteIds).slice(0, 5)) {
      ...
      console.log(`   Cotizaciones (${allCots.length}):`, allCots.map(ct => `${ct.codigo} (S/ ${ct.monto})`).join(', ') || 'Sin cotizaciones');
